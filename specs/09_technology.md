...
- correction: 2026-09-23. The Verified row named EnSight Gold and VTKHDF while this build read
  neither: `src/engine/reader.py` had no reader for `.case` or `.vtkhdf`, so a file of either opened
  as Absent - "no reader for this format in this build". The gap was held by name in the regression
  set's gate (XC-307, `tests/test_regression_catalogue.py`) and closed the same day: XC-308 wired
  both, with every file an EnSight case names checked against its own counts before the toolkit's
  reader sees it, because that reader measured as ending the process, not returning, or reading a
  cut file as whole (E-227). The row is true of this build; this line is the record that it was not
...
| ANARI-SDK (Khronos) | the interface this product renders through, so the renderer is a back end (XC-251) | Apache-2.0 (E-156) | Khronos standard; AMD, Intel and NVIDIA each ship a device (E-155) | Khronos; 1.0 since 2023-08, 1.1 in preview | writing to one renderer's interface, which makes that vendor a dependency |
| Intel OSPRay + Embree + Open VKL + Open Image Denoise | the CPU ray-traced path: surfaces, **unstructured volumes**, isosurfaces, denoising | Apache-2.0, all four (E-154, E-156) | Intel's own; the renderer behind ParaView's ray-traced display | Intel `RenderKit` | a GPU-only path, which XC-086's laptop class cannot run |
| NVIDIA VisRTX | the RTX ray-traced path, reached through ANARI and **not** through Omniverse | BSD-3-Clause (E-156) | NVIDIA's own ANARI device | NVIDIA | the Omniverse Kit path, which may not be redistributed at all (E-151) |
