...
|---|---|---|---|
| ビュー（`比較` when a @Comparison is open） | name and description; the comparison definition when the open item is one; guides - world axes, ground grid, orientation gizmo, scale bar, selection outline; unit display. **Not the split**: the pane count and camera synchronisation are session state set from the work area bar, and each pane's @Case and camera are on that pane's badge (XC-202, XC-204, XC-209) | `View.guides`, `View.comparison` | a pane whose @Case has no @Dataset names that, and draws nothing |
...
| Section | Elements | Bound to | Unresolved |
|---|---|---|---|
| グラフ | name; title; subtitle; caption; the **kind** - 2D line, scatter, bar, histogram, box/quartile, parallel coordinates, plot matrix, heatmap; 3D surface, scatter, contour, with projection and view direction - and legend on/off, position, symbol width and **which @Case the graph covers** - selection (CT-007), iteration handling, reduction and its weighting quantity - because a series spans every selected case (XC-221) | `Graph.title`, `Graph.kind`, `Graph.legend`, `Graph.caseSelection`, `Graph.reduction` | a kind the selected quantities cannot support names why |
| 系列 | the series list with add, remove, reorder; @Reference material. **Per series, on its own row**: label, X source (@Result axis, a parameter, index), Y quantity or @Expression, @Declared unit, @Provenance, missing-value policy, target axis, colour, line style, thickness, marker style and size, opacity - appearance is keyed to the series rather than to the chart (XC-213, E-124), and each appearance field defers to the theme unless the series overrides it, drawn as the theme resolves it (XC-221) | `Graph.series[]` | a series with no quantity is listed and drawn as no data, never dropped from the legend; a selection that resolves to nothing says which condition emptied it (CT-007) |
| 軸 | **one axis chosen** - X, Y, second Y - and the same fields serve whichever it is: title, @Declared unit annotation, range data-fitted or fixed, log scale, tick interval, notation and precision, custom labels, grid. The measured reference repeats 25 properties for each of four axes; choosing the axis is the same reach in a twentieth of the panel (XC-213, E-124) | `Graph.axes` | an axis whose series have no declared unit carries the undeclared marker, never an assumed label (XC-003); a fixed range that excludes data states that on the figure and in its export |
| スタイル | style @Asset, and a **read-out** of what that asset supplies a series that does not override it - line width and marker - because there is no chart-wide level between the asset and the series (XC-224, E-124); palette chosen for discriminability; background; typography | `Graph.style` | - |
...
| Section | Elements | Bound to | Unresolved |
|---|---|---|---|
| レポート | name; document title; language; source @Template and its revision; the mandatory trust content - @Provenance, @Declared unit, limitations, product version - each shown as present and not removable | `Report.identity`, `Report.trustContent` | a mandatory item that cannot be produced blocks export and names itself |
| 内容 | which @Workspace and which @Case; @Reference material; the ordered block list with per-block reference and, for a @View block, still, video or interactive | `Report.blocks[]` | a block whose referenced item was deleted is listed as unresolved and is not silently dropped |
| 執筆 | the method - mechanical summary, or generated; for generated the direction, depth, model and whether a search may be made; then the action that produces a draft, and the state of that draft: not made, awaiting review, taken in. A sequence rather than a set of settings, because the measured tool produces an outline before any content and generates on the user's word (XC-214, E-126). An unset model blocks the action rather than annotating it | `Report.commentary` | nothing enters the document until each statement has been seen with its kind and its source (XC-104) |
| スタイル | the document **theme**, in one place: page size, orientation, margins, columns; header, footer, page numbers; figure width; @Art style asset; palette including a monochrome print variant; table rules; figure treatment; body and heading typeface, sizes, embedding scope | `Report.layout`, `Report.artStyleId`, `Report.typography` | a glyph the chosen face cannot draw is named with the element that used it, never exported as an empty box |
