...
| 系列 | the series list with add, remove, reorder; @Reference material. **Per series, what it plots**: label, X source (@Result axis, a parameter, index), Y quantity or @Expression, @Declared unit, @Provenance, missing-value policy, and the axis it is read against. **How it looks is in `スタイル`** (XC-226), still keyed to the series and addressed by the same selection | `Graph.series[]` | a series with no quantity is listed and drawn as no data, never dropped from the legend; a selection that resolves to nothing says which condition emptied it (CT-007) |
...
| スタイル | style @Asset, and a **read-out** of what that asset supplies a series that does not override it - line width and marker - because there is no chart-wide level between the asset and the series (XC-224, E-124); palette chosen for discriminability; background; typography; and **the selected series' colour, line, width and marker** (XC-226) | `Graph.style`, `Graph.series[]` | - |
