...
| Section | Elements | Bound to | Unresolved |
|---|---|---|---|
| グラフ | name; title; subtitle; caption; the **kind** - 2D line, scatter, bar, histogram, box/quartile, parallel coordinates, plot matrix, heatmap; 3D surface, scatter, contour, with projection and view direction - and legend on/off, position, symbol width | `Graph.title`, `Graph.kind`, `Graph.legend` | a kind the selected quantities cannot support names why |
| データ | case selection (CT-007) - selected, saved, declarative condition, or Python; iteration handling; reduction method and its weighting quantity; @Reference material; and the series list with add, remove, reorder. **Per series, on its own row**: label, X source (@Result axis, a parameter, index), Y quantity or @Expression, @Declared unit, @Provenance, missing-value policy, target axis, colour, line style, thickness, marker style and size, opacity - appearance is keyed to the series rather than to the chart (XC-213, E-124) | `Graph.series[]`, `Graph.caseSelection`, `Graph.reduction` | a series with no quantity is listed and drawn as no data, never dropped from the legend; a selection that resolves to nothing says which condition emptied it (CT-007) |
...
| スタイル | style @Asset; palette chosen for discriminability; background; the defaults a new series starts from; typography | `Graph.style` | - |
