...
- decision: the repository is published to
  `take-works-tech/261SV`, and to nowhere else. Pushing, adding a
...
- correction: 2026-08-30, "private" removed from the decision by the product owner: the repository
  is now **public**. The reasons align rather than compete - the owner hit the private-plan GitHub
  Actions ceiling; XC-082 had already fixed source-available publication as the product's position,
  and publishing the repository is that decision executed; and OPEN-020's blocker ("branch protection
  requires Pro or a public repository", E-129) falls away. Before the switch the tree was audited:
  the two private planning notes under evidence/source-notes were removed, six unreferenced template
  images were removed, and the twelve referenced mockup images - which had no recorded origin - were
  regenerated by `mockups/ui/scripts/generate_placeholder_assets.py`, so every asset's provenance is
  now this repository. **The git history keeps the original bytes of the replaced images**: history
  rewriting is refused by this same item, so publication publishes them too. They are generic
  gradient/silhouette swatches and the judgement is that this is acceptable; if any of them is ever
  shown to be third-party material, the remedy is a fresh repository, decided then, not a rewrite.
  LICENSE.md now carries FSL-1.1-MIT (E-159), the licence XC-082 decided
...
### XC-298 - The sample is generated by the engine where the person chose, opened with its first case drawn, and never copied from the installation
- decided: 2026-09-21
- status: active
- decision: the shipped sample of XC-129 is **code, not files**: `workspace.sample` (CT-003 3.18.0)
  writes, where the person chose, a document named 片持ち梁（サンプル） with two cases - a cantilever
  beam under a tip load of 100 N and of 150 N, tagged 静荷重 and 1.5 倍 - each with one `.vtu` the
  engine generates from Euler-Bernoulli beam theory (`engine/sample.py`: a metre long, a tenth high,
  a twentieth wide, steel-like, 40 x 4 x 2 hexahedra; bending stress and deflection as point
  fields, the stress at cell centres as a cell field, float32 like a solver's output), in a folder
  beside the document; the units the file cannot carry are declared in the document by the sample's
  author, this product (XC-003), so the first picture has a legend in pascal. The engine then opens
  it and answers as `workspace.open` does - each case now with its recorded files, where they are
  and whether they are there - and the interface loads the first case's file so the first thing seen
  is the beam. Refused where the document or its data folder already exists: a sample a person
  changed is theirs, and a newer build writes a newer sample under another name (XC-130). The
  Workspace list offers the sample first while nothing was opened yet. Not this decision's: the
  first launch landing in the View area on its own (operations/AC-001, operations/TASK-006 stays open) and the
  tutorial
- decided_by: engineering judgement, from #237's condition, XC-129 and XC-085
- rationale: a sample that is generated carries no third-party terms and no stale bytes, and every
  number in it follows from five constants and two formulas, so what the product reports about it
  can be checked by a reader with a pencil - the maximum stress at the clamped end is `F L (h/2) / I`
  = 1.2 MPa under 100 N (E-220). Writing it where the person chose keeps the installation read-only
  and the person's copy theirs, which is XC-130's whole content. Two loads rather than one, so the
  case tree, the subject badge and a graph over cases have something to show
- alternatives: **files under `resources/` copied on request** - the same bytes in two places, a
  copy to keep from overwriting, and a licence question for data that would still be this
  project's. **A sample opened in place from the installation** - read-only, so the first save
  fails. **A single case** - a tree of one is not a tree
- basis: E-220 (T1)
- affects: MOD-002, MOD-007, MOD-012, MOD-016, CT-003, operations/REQ-001
- decidedness: Fixed
