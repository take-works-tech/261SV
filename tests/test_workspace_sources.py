...
class TestWhatIsRecorded:
    def test_a_reference_holds_the_path_size_and_time_and_not_the_contents(self, tmp_path: Path) -> None:
        ...
        source = record(a_file(tmp_path, "run.vtu"), relative_to=tmp_path, where=FIXED_MOMENT)

        assert set(source) == {"pathRelative", "sizeBytes", "modified"}
    ...
    def test_the_path_is_relative_so_a_moved_project_still_opens(self, tmp_path: Path) -> None:
        ...
        source = record(a_file(tmp_path / "inputs", "run.vtu"), relative_to=tmp_path, where=FIXED_MOMENT)
...
class TestThreeStatesRatherThanTwo:
    ...
    def test_a_deleted_file_is_missing(self, tmp_path: Path) -> None:
        ...
        source = record(path, relative_to=tmp_path, where=FIXED_MOMENT)
        path.unlink()
...
class TestACaseOpensUnresolvedRatherThanFailing:
    def test_every_source_present_resolves_the_case(self, tmp_path: Path) -> None:
        ...
        case["sources"] = [record(a_file(tmp_path, f"{n}.vtu"), relative_to=tmp_path, where=FIXED_MOMENT) for n in "ab"]
    ...
    def test_one_missing_source_leaves_the_case_unresolved(self, tmp_path: Path) -> None:
        ...
        case["sources"] = [record(path, relative_to=tmp_path, where=FIXED_MOMENT) for path in paths]
        paths[0].unlink()
