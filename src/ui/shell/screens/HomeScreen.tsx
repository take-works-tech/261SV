...
function RecentWorkspaces() {
  ...
  // The shipped sample: generated by the engine where the person chooses, so nothing is copied
  // from the installation and a sample a person changed is theirs (XC-129, XC-130, XC-298).
  const sample = async () => {
    const path = await shell.dialog.saveWorkspace("片持ち梁（サンプル）.svw");
    if (!path) return;
    setProblem(null);
    if (!(await engineState.openSample(path))) {
      setProblem({ path, because: snapshot().refusal ?? "サンプルを作れませんでした" });
      return;
    }
    setEntries(await shell.recent.list());
    session.openWorkspace();
      ...
      <div className="ho-inner">
        <header className="ho-head">
          ...
          <div className="ho-tools">
            ...
            <button className="btn" onClick={() => void sample()} disabled={e.busy} title="片持ち梁のサンプル（自前生成、二ケース、単位宣言済み）を選んだ場所に書いて開きます（workspace.sample）">サンプルを開く</button>
        ...
        {entries === null ? (
          ...
          <div className="ho-empty">
            ...
            <p>まずサンプルを。片持ち梁の二ケースをこの製品自身が生成し（第三者のデータではありません）、選んだ場所に書いて、最初のケースを描いたところから始めます。「＋ 新規作成」は一つのケースを持つ空の文書、既にある .svw は「開く…」かウィンドウへのドロップで開けます。</p>
            <div className="actions">
              <button className="btn primary" onClick={() => void sample()} disabled={e.busy}>サンプルを開く</button>
              <button className="btn" onClick={() => void create()} disabled={e.busy}>＋ 新規作成</button>
              <button className="btn" onClick={() => void pick()} disabled={e.busy}>開く…</button>
            </div>
          </div>
        ) : visible.length === 0 ? (
